...
class Window(QMainWindow):
    ...
    def paintSolar(self, painter, earthTrajectoryRadius, moonTrajectoryRadius):
        ...
        # draw earth
        earthCenter = QPoint(
            earthTrajectoryRadius *
            cos(self.earthAngle/180.0*3.14) + center.x(),
            earthTrajectoryRadius *
            sin(self.earthAngle/180.0*3.14) + center.y()
        )
        painter.drawPixmap(earthCenter.x()-40,
                           earthCenter.y()-40, 80, 80, self.earthBmp)

        # draw moon
        moonCenter = QPoint(
            moonTrajectoryRadius *
            cos(self.moonAngle/180.0*3.14) + earthCenter.x(),
            moonTrajectoryRadius *
            sin(self.moonAngle/180.0*3.14) + earthCenter.y()
        )
        painter.drawPixmap(moonCenter.x()-10,
