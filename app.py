...
import sys
from PyQt5.QtWidgets import QMainWindow, QApplication, QWidget, QPushButton, QAction, QBoxLayout
...
class App(QMainWindow):   
    ...
    def initUI(self):
        ...
        for alias in modules: 
            if alias != 'name': syllabus.append(alias)
        ...
        for syl in syllabus:
            ...
            for alias in modules[syl]: 
                if alias != 'name': exercises.append(alias)
            # traverse through exercises in each syllabus
            for sub in exercises:
                exerciseSelectAction = QAction(getattr(modules[syl][sub], 'name', sub), self)
                # connect to actions
